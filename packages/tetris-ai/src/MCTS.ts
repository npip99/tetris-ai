...
class MCTS {
    ...
    simulate(cPuctInit: number, cPuctBase: number) {
        ...
        if (currentNode.isFinalState) {
            ...
            this.pendingSimulations.push({
                ...
                path: path,
                tensorInput: currentNode.gameState.toTensor().transpose([1, 2, 0]),
            });
        }
    }
    ...
    // Returns the tensors it wants evaluated,
    // Or [] if it's done simulating
    iterate(): tf.Tensor3D[] {
        ...
        if (!this.initializedRootNode) {
            ...
            let numValidActions = this.rootNode.validActions.reduce((a, b) => a + (b ? 1 : 0), 0);
            for(let i = 0; i < this.game.getNumActions(); i++) {
        ...
        for(let i = 0; i < this.args.numParallelSims && this.rootNode.numVisits < this.args.numMCTSSims; i++) {
            this.simulate(cPuctInit, cPuctBase);
        }
