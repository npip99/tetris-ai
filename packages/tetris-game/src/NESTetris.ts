...
class NESTetrisGame {
    ...
    clearLines() {
        ...
        // Track total lines cleared, and the score
        this.totalLinesCleared += this.lastClearedLines;
        // Level up, before counting the score for that linecount
    ...
    handleShift() {
        ...
        let will_shift_piece = false;

        if (just_pressed) {
            this.dasHorizontal = 0;
            will_shift_piece = true;
        } else {
            this.dasHorizontal++;
        ...
        if (will_shift_piece) {
            if (this.buttonState[Buttons.RIGHT] != ButtonState.RELEASED) {
                if (!this.board.tryMovePiece(this.current_piece, 1, 0)) {
                    // If it can't move, prime DAS
                    this.dasHorizontal = 16;
                }
            } else if (this.buttonState[Buttons.LEFT] != ButtonState.RELEASED) {
                if (!this.board.tryMovePiece(this.current_piece, -1, 0)) {
                    // If it can't move, prime DAS
                    this.dasHorizontal = 16;
                }
    ...
    iterate() {
        ...
        } else if (this.ARE_counter == 5 || this.ARE_counter == 6 || this.ARE_counter == 7) {
